...
import { Tree, TAB_WIDTH } from "./components";
...
describe("Shell — EditorArea wired to a DocumentManager (Req 6.5, 6.6, design.md §8.1)", () => {
  ...
  test("clicking the second tab's column switches the active document's content (Issue #138)", async () => {
  ...
  test("clicking past the last tab's column leaves the active document unchanged (Issue #138)", async () => {
    ...
    // throw or fall back to some other tab.
    const { slotRegistry, layoutState, context } = createHarness();
    await layoutState.ready;
    const documents = createDocumentManager({
      log: createHostLog(),
    ...
    const tabSelect = findTabSelect(renderer.root);
    expect(tabSelect).toBeDefined();
    // Only 2 tabs are open (columns `[0, 2 * TAB_WIDTH)` relative to the
    // strip); this lands well past both.
    const outOfRangeColumn = tabSelect!.x + TAB_WIDTH * 5;
    act(() => {
      tabSelect!.processMouseEvent(
        new OpenTuiMouseEvent(tabSelect!, {
          type: "down",
