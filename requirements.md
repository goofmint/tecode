...
6. WHEN tecode is launched with `--theme <file>` (Issue #149), THE system SHALL load that theme JSON file directly and activate it immediately, independent of the `workbench.colorTheme` setting and without writing to any settings file. A subsequent live `workbench.colorTheme` change (whether a hand-edit or `theme.select`'s own commit) SHALL NOT replace the `--theme` override; the override SHALL remain active until tecode is restarted without the flag. An explicitly named `--theme` file that does not exist, cannot be read, or is not valid JSON with a top-level object structure SHALL be treated as a fatal startup error, consistent with `--settings`/`--keybindings`'s own fatal-on-malformed-content policy (Req 9.7); a syntactically valid theme object whose contents are otherwise incomplete or unrecognized (e.g. an unknown color key) SHALL still register and gracefully degrade per-key to the base palette, exactly like every other theme source (built-in, extension-declared, user-themes-directory — Req 7.1).
...
7. WHEN tecode is launched with `--settings <file>` and/or `--keybindings <file>` (Issue #149), THE system SHALL layer each named file's contents on top of the home-directory (or `--config <dir>`-overridden) user layer, as a new CLI layer — the two flags compose with `--config <dir>` rather than replacing it. For `--settings`, this CLI layer SHALL sit below the workspace settings layer (`defaults ← user ← CLI ← workspace` — a workspace's own `.tecode/settings.json` still wins). Keybindings have no workspace-level file at all, so this requirement does not apply to them the same way: for `--keybindings`, the CLI layer SHALL be the single highest-precedence layer of all (`defaults → fallback → extension → user → CLI`), above even the user's own `keybindings.json`. An explicitly named file that does not exist, cannot be read, or is not valid JSONC in the expected top-level shape (an object for `--settings`, an array for `--keybindings`) SHALL be treated as a fatal startup error, unlike `--config <dir>`'s tolerant "missing file is an empty layer" policy. Once tecode has started, THE system SHALL watch each explicitly named `--settings`/`--keybindings <file>` for changes and apply a valid edit to it immediately, without restarting, exactly like the home-directory/workspace settings and keybindings files (Req 9.4); an edit that makes the file unreadable or invalid at that point SHALL keep the CLI layer's last-good content (not the fatal-at-startup treatment above, which applies only to the very first load) and SHALL be reported as a warning, consistent with every other settings/keybindings file's own live-reload failure handling. A settings key set by the CLI layer SHALL NOT be silently overwritten by a write-back that would have no visible effect (e.g. `workbench.sidebarWidth`'s resize-commit persistence) — such a write-back SHALL be skipped with a warning instead.
