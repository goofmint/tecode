...
7. WHEN tecode is launched with `--settings <file>` and/or `--keybindings <file>` (Issue #149), THE system SHALL layer each named file's contents on top of the home-directory (or `--config <dir>`-overridden) user layer, as a new CLI layer sitting below the workspace layer (`defaults ← user ← CLI ← workspace`) — the two flags compose with `--config <dir>` rather than replacing it. An explicitly named file that does not exist, cannot be read, or is not valid JSONC in the expected top-level shape (an object for `--settings`, an array for `--keybindings`) SHALL be treated as a fatal startup error, unlike `--config <dir>`'s tolerant "missing file is an empty layer" policy. A settings key set by the CLI layer SHALL NOT be silently overwritten by a write-back that would have no visible effect (e.g. `workbench.sidebarWidth`'s resize-commit persistence) — such a write-back SHALL be skipped with a warning instead.
