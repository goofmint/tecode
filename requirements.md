...
7. WHEN tecode is launched with `--settings <file>` and/or `--keybindings <file>` (Issue #149), THE system SHALL layer each named file's contents on top of the home-directory (or `--config <dir>`-overridden) user layer, as a new CLI layer sitting below the workspace layer (`defaults ← user ← CLI ← workspace`) — the two flags compose with `--config <dir>` rather than replacing it. An explicitly named file that does not exist, cannot be read, or is not valid JSONC in the expected top-level shape (an object for `--settings`, an array for `--keybindings`) SHALL be treated as a fatal startup error, unlike `--config <dir>`'s tolerant "missing file is an empty layer" policy. Once tecode has started, THE system SHALL watch each explicitly named `--settings`/`--keybindings <file>` for changes and apply a valid edit to it immediately, without restarting, exactly like the home-directory/workspace settings and keybindings files (Req 9.4); an edit that makes the file unreadable or invalid at that point SHALL keep the CLI layer's last-good content (not the fatal-at-startup treatment above, which applies only to the very first load) and SHALL be reported as a warning, consistent with every other settings/keybindings file's own live-reload failure handling. A settings key set by the CLI layer SHALL NOT be silently overwritten by a write-back that would have no visible effect (e.g. `workbench.sidebarWidth`'s resize-commit persistence) — such a write-back SHALL be skipped with a warning instead.
