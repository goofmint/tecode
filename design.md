...
1. **Synchronous, before first frame (< 100 ms budget):** parse argv (including `--settings`/`--keybindings`/`--theme <file>`, Req 9.7/7.6, Issue #149 — each explicitly-named file is verified readable here, before anything else, since an unreadable one is a fatal startup error); load user + CLI + workspace settings and keybindings (JSONC); detect terminal capabilities (Kitty protocol, color depth); build the theme from the configured theme's cached JSON — `themes-default` embeds only Dark Modern's JSON as a compiled-in asset (Issue #124: every other theme, including one equivalent to Light Modern, ships as a plain file under the top-level `themes/` and the user themes directory instead), so painting Dark Modern for the very first frame still needs no extension activation; render the Shell with empty slots and a placeholder editor area. This embedded-default-theme resolution is a hard contract this task preserves exactly: it MUST stay synchronous (no `await` between `buildAssemblyRoot`'s theme wiring and `renderShell`) and MUST keep `themeLoader.ts`'s per-key degrade-to-base-palette policy for any key the embedded JSON omits — no frame is ever painted with the unresolved base palette in place of Dark Modern. Immediately after this ordinary theme application, a `--theme <file>` override (if given) is loaded and activated directly, taking over as the active theme before the first frame renders (§9).
2. **Deferred (queueMicrotask after first frame):** discover extensions and validate manifests; register declared contributions; fire `onStartup` activations; scan the user themes directory (`~/.config/tecode/themes/*.json`, Issue #124) and register each file the same way as a manifest's `contributes.themes` entry, re-applying `workbench.colorTheme` once that scan settles so a user theme named there actually activates; open the file/directory from argv (which fires `onLanguage:*` activations).
...
- **Embedded vs. file-based themes** (Issue #124): only ONE theme's JSON is an embedded asset baked into the binary — `themes-default`'s Dark Modern (`packages/builtin/themes-default/assets.ts`), served through `packages/cli`'s `themeAssetsFs.ts` overlay for `ThemeRegistry`'s filesystem seam so its synthetic `<builtin>/<id>` path resolves in both `bun run` and a `bun build --compile` binary. This embedded-default-theme contract is preserved exactly by this task: Dark Modern resolves SYNCHRONOUSLY, ahead of the first frame (§3), and any key it omits still degrades per-key to the base palette via `themeLoader.ts` — never a whole-frame fallback. Every other theme — including one equivalent to Light Modern, which used to be `themes-default`'s second embedded contribution — is an ordinary JSON FILE: the repository ships copyable examples at its top-level `themes/`, and `packages/cli/src/userThemes.ts` scans the user themes directory (`~/.config/tecode/themes/`, `host/paths.ts`'s `getUserThemesDir`) during the deferred phase, turning each `*.json` file into a `PendingThemeContribution` fed into the SAME `ThemeRegistry.loadContributions` call a manifest-declared theme goes through — no second distribution or loading mechanism. A user theme's `path` resolves through `ThemeRegistry`'s ordinary real `fs.readFile`, never the embedded-asset overlay (that overlay only intercepts the one built-in's synthetic path); a missing user themes directory or an individually unreadable file degrades to "this theme just isn't registered," while a readable-but-malformed file still registers and degrades to the base palette per `themeLoader.ts`'s existing whole-file-parse-failure policy — startup itself never fails either way.
...
- **`--theme <file>` direct override** (*Req 7.6*, Issue #149): a theme JSON file named on the CLI (`cli/argv.ts`'s `resolveThemeFileOverride`) is loaded and activated directly (`cli/userThemes.ts`'s `loadThemeFileOverride` builds one `PendingThemeContribution`, fed into the SAME `ThemeRegistry.loadContributions` call every other theme goes through, with an empty `extensionDirs` map since its path is already absolute), independent of `workbench.colorTheme`, right after `runTecode`'s ordinary `applyConfiguredTheme` call so it always wins over whatever `settings.json` names. `AssemblyRoot.cliThemeOverride` (a shared mutable `{ active: boolean }` ref) flips to `true` once activated; `ui/themeConfigSync.ts`'s `wireThemeConfigSync` and `ui/themeSettingsWriter.ts`'s writer both consult it to suppress, respectively, a live `workbench.colorTheme` re-application and a `theme.select` commit's write-back while the override is active — the loaded file's id/label come from its filename stem and its own top-level `"name"`, exactly like a user-themes-directory file (§9 above). An unreadable `--theme` file is a fatal startup error, like `--settings`/`--keybindings` (§11).
...
- **`--settings <file>`/`--keybindings <file>` CLI layer** (*Req 9.7*, Issue #149): unlike `--config <dir>`'s "redirect the user layer" shape, these two flags (`cli/argv.ts`'s `resolveSettingsFileOverride`/`resolveKeybindingsFileOverride`) each name a SEPARATE, higher-precedence layer — `ConfigServiceDeps.cliSettingsPath`/`cliKeybindingsPath` (`config/service.ts`) — but the two layers sit at DIFFERENT points in their respective precedence orders, since only settings has a workspace layer at all: `--settings`'s CLI layer sits above the user layer but below the workspace layer (`defaults ← user ← CLI ← workspace`); `--keybindings`'s CLI layer feeds a new `cli` layer in `keymap/bindingTable.ts`'s `KeymapLayers` (`LAYER_ORDER`'s highest-precedence entry, above `user` — keybindings have no workspace-level file to sit below) via `keymapState.ts`'s `setCliEntries`. The two flag families compose with `--config <dir>`: `--config <dir> --settings <file>` layers `<file>` on top of `<dir>/settings.json`. Both files are watched for live reload exactly like every other layer's file. Unlike a missing `--config <dir>` file (an empty layer, no error), an EXPLICITLY named `--settings`/`--keybindings` file that cannot be read is a fatal startup error (`cli/main.ts`'s `runTecode`, before `buildAssemblyRoot` is even called) — a typo in an explicit flag should never be silently ignored. `ConfigService.isSetByCliLayer(key)` lets a settings writer (`ui/sidebarWidthSettingsWriter.ts`) suppress a write-back that would succeed on disk but never be visibly reflected, because the CLI layer masks it right back out on the next read.
...
- **themes-default / languages-basic**: pure-contribution extensions (no `activate` logic beyond registration); `languages-basic`'s WASM/scm files are embedded assets, and so is `themes-default`'s one remaining contribution (Dark Modern's JSON, Issue #124 — every other theme is a plain file under the top-level `themes/`/user themes directory, §9).
